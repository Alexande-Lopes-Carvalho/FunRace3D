...
public class PathEditor : Editor {
    public void OnSceneGUI(){
        ...
        if(path.RawPoints.Count <= 1){
            return;
        }
        List<Vector3> raw = new List<Vector3>(), p = new List<Vector3>();
        for(int i = 0 ; i < path.RawPoints.Count; ++i){
            raw.Add(path.RawPoints[i].x*path.transform.right + path.RawPoints[i].y*path.transform.up + path.RawPoints[i].z*path.transform.forward + path.transform.position);
        }
        ...
        for(int i = 0; i < raw.Count; ++i){
            ...
            if(EditorGUI.EndChangeCheck()){
                ...
                Vector3 res = newPosition-path.transform.position;
